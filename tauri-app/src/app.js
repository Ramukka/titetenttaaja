...
const resolveAssetUrl = async (relativePath) => {
  const normalized = relativePath.replace(/^\.?\//, "");
  if (window.__TAURI__?.core?.getAssetUrl) {
    try {
      return await window.__TAURI__.core.getAssetUrl(normalized);
    } catch (error) {
      console.error("Asset URL resolution failed, falling back to relative path", error);
    }
  }
  return `./${normalized}`;
};
...
async function loadQuiz(examId) {
  ...
  const assetUrl = await resolveAssetUrl(manifestEntry.path);
